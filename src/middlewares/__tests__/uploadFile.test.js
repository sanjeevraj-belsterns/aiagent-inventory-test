...
describe('commonUploadMiddleware', () => {
    ...
    it('should call next() when form data is parsed successfully', () => {
        multer.mockImplementation(() => ({ any: () => (req, res, cb) => cb(null) }));
        commonUploadMiddleware(req, res, next);
        expect(next).toHaveBeenCalled();
    });

    it('should return a 400 error when there is an error parsing form data', () => {
        const error = new Error('Parsing error');
        multer.mockImplementation(() => ({ any: () => (req, res, cb) => cb(error) }));
        commonUploadMiddleware(req, res, next);

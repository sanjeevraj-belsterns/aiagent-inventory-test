...
describe('User Controller', () => {
  ...
  describe('logIn', () => {
    it('should log in the user with correct credentials', async () => {
      const req = { body: { email: 'test@example.com', password: 'password123' } };
      const res = { status: jest.fn().mockReturnThis(), send: jest.fn() };
      const user = { firstName: 'Test', lastName: 'User', email: 'test@example.com', password: 'hashedPassword', _id: '123', role: 'user' };
    ...
    it('should return an error for incorrect password', async () => {
      ...
      const res = { status: jest.fn().mockReturnThis(), send: jest.fn() };
      const user = { firstName: 'Test', lastName: 'User', email: 'test@example.com', password: 'hashedPassword' };
      UserModel.findOne.mockResolvedValue(user);

...
describe('User Controller', () => {
  describe('signIn', () => {
    it('should create a new user if the user does not exist', async () => {
      ...
      expect(ts.hashPassword).toHaveBeenCalledWith('password123');
  ...
  describe('logIn', () => {
    it('should log in the user with valid credentials', async () => {
      ...
      expect(UserModel.findOne).toHaveBeenCalledWith({ email: 'test@example.com' });
      expect(ts.hashCompare).toHaveBeenCalledWith('password123', 'hashedPassword');
      expect(ts.createToken).toHaveBeenCalledWith({ firstName: 'Test', lastName: 'User', email: 'test@example.com', id: '1', role: 'user' });
      expect(res.status).toHaveBeenCalledWith(200);

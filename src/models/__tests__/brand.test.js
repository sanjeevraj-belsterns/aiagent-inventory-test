import mongoose from 'mongoose';
import BrandModel from '../brand.js';

describe('Brand Model', () => {
    beforeAll(async () => {
        await mongoose.connect('mongodb://localhost:27017/test', { useNewUrlParser: true, useUnifiedTopology: true });
    });

    afterAll(async () => {
        await mongoose.connection.close();
    });

    it('should create a brand with valid data', async () => {
        const brand = new BrandModel({ brandImage: 'image.png', brandName: 'Test Brand' });
        const savedBrand = await brand.save();
        expect(savedBrand._id).toBeDefined();
        expect(savedBrand.brandName).toBe('Test Brand');
    });

    it('should not create a brand without a name', async () => {
        const brand = new BrandModel({ brandImage: 'image.png' });
        let error;
        try {
            await brand.save();
        } catch (err) {
            error = err;
        }
        expect(error).toBeDefined();
        expect(error.errors.brandName).toBeDefined();
    });

    it('should not create a brand name longer than 20 characters', async () => {
        const brand = new BrandModel({ brandImage: 'image.png', brandName: 'This brand name is way too long' });
        let error;
        try {
            await brand.save();
        } catch (err) {
            error = err;
        }
        expect(error).toBeDefined();
        expect(error.errors.brandName).toBeDefined();
    });
});